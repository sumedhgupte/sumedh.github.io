...
Mathematically speaking, this _goodness_ is simply a real valued function defined on the set of observations. Making use of the fact that the observations themselves originate from one another, and thus their values may exhibit some **relation**, we can specifically focus on **recursive functions**. 
